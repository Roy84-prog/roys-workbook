            ...
            for key in ["Num", "Eng", "Kor", "Lib", "Easy", "Mark", "Context", "Tip"]:
                ...
                if match:
                    ...
                    if key in ["Eng", "Kor", "Lib"]:
                        ...
                        # 2. 순서배열 (A), (B), (C) 제거 (문장 맨 앞 단독 등장 시)
                        val = re.sub(r'^\s*\([A-C]\)\s*', '', val)

                        # 3. 💡 핵심 수정: 번호(❶~⓴) 뒤에 (A), (B), (C)가 붙는 경우 안전하게 제거
                        # 유니코드 에러를 방지하기 위해 특수문자를 모두 명시적으로 지정했습니다.
...
def process_folder(target_folder, output_folder):
    ...
    txt_files = [f for f in glob.glob(os.path.join(input_path, "*.txt")) if not f.endswith(".json")]
    if not txt_files:
