...
# [설정 영역]
# ==========================================
OUTPUT_FILENAME = "Roy_English_Material_Final_v34.html"
...
/* Subject & Feature (Yellow) */
.highlight-subject, .highlight-feature {{ background-color: #fff9c4; border-bottom: 2px solid #fbc02d; padding-left:2px; padding-right:2px; }}
...
/* Clue (Blue) */
.highlight-clue {{ background-color: #e3f2fd; border-bottom: 2px solid #64b5f6; padding-left:2px; padding-right:2px; }}
...
/* [Header Info Badges] */
.header-badge-them {{
    font-size: 12px; background-color: #ffebee; color: #c62828; border: 1px solid #ef9a9a;
    padding: 2px 8px; border-radius: 12px; margin-left: 8px; vertical-align: middle; font-weight: bold;
}}
.header-badge-key {{
    font-size: 12px; background-color: #fffde7; color: #f57f17; border: 1px solid #fff59d;
    padding: 2px 8px; border-radius: 12px; margin-left: 5px; vertical-align: middle; font-weight: bold;
}}

/* [Interlinear Vocab] */
.vocab-container {{ display: inline-flex; flex-direction: column; align-items: center; vertical-align: text-top; margin: 0 2px; line-height: 1.1; }}
.vocab-text {{ font-weight: normal; border-bottom: 1px dotted #888; color: inherit; }}
.vocab-sub {{ font-size: 11px; color: #d32f2f; margin-top: 1px; font-weight: normal; font-family: var(--font-kor); }}

/* [Syntax Parsing Style] - 밑줄 및 라벨 */
.syntax-box {{
    display: inline; /* 줄바꿈 허용 */
    border-bottom: 2px solid; /* 밑줄 */
    position: relative;
    padding-bottom: 1px;
    margin-right: 3px;
}}
...
/* S (주어) - 파랑 */
.S-box {{ border-bottom-color: var(--c-S); }}
.S-box::before {{ content: 'S'; color: var(--c-S); }}

/* V (동사) - 빨강 */
.V-box {{ border-bottom-color: var(--c-V); }}
.V-box::before {{ content: 'V'; color: var(--c-V); }}

/* O (목적어) - 갈색 */
.O-box {{ border-bottom-color: var(--c-O); }}
.O-box::before {{ content: 'O'; color: var(--c-O); }}

/* C (보어) - 초록 */
.C-box {{ border-bottom-color: var(--c-C); }}
.C-box::before {{ content: 'C'; color: var(--c-C); }}

/* M (수식어) - 회색 */
.M-box {{ border-bottom-color: var(--c-M); }}
.M-box::before {{ content: 'M'; color: var(--c-M); }}

/* Conn (접속사) - 보라 */
.con-box {{ border-bottom-color: var(--c-con); }}
.con-box::before {{ content: 'Conn'; color: var(--c-con); }}
...
/* [Options] */
.options-box {{ margin-top: 20px; padding: 20px; border: 1px solid #eee; border-radius: 8px; background: #fcfcfc; }}
.option-item {{ margin-bottom: 14px; font-size: 15px; border-bottom: 1px dashed #eee; padding-bottom: 10px; }}
.option-item:last-child {{ border-bottom: none; }}
.opt-text {{ font-family: var(--font-eng); font-weight: bold; font-size: 16px; margin-right: 8px; color: #333; }}
.err-badge-wrong {{ font-size: 11px; background-color: #fff3e0; color: #ef6c00; padding: 2px 6px; border-radius: 4px; border: 1px solid #ffe0b2; margin-right: 6px; font-weight: bold; vertical-align: 1px; }}
.err-badge-correct {{ font-size: 11px; background-color: #e8f5e9; color: #2e7d32; padding: 2px 6px; border-radius: 4px; border: 1px solid #a5d6a7; margin-right: 6px; font-weight: bold; vertical-align: 1px; }}
.opt-reason {{ font-size: 14px; color: #333; font-weight: normal; }}
.opt-trans {{ display: block; font-size: 13px; color: #888; margin-top: 4px; margin-left: 5px; }}

/* Analysis Table */
table.analysis-table {{ width: 100%; border-collapse: collapse; margin-bottom: 25px; font-size: 13px; table-layout: fixed; }}
table.analysis-table th {{ background: #f1f3f5; padding: 8px; border: 1px solid #ddd; text-align: center; font-weight: bold; color:#555; }}
table.analysis-table td {{ padding: 8px; border: 1px solid #ddd; vertical-align: middle; word-wrap: break-word; background: #fff; }}
.table-label {{ font-weight: bold; font-size: 15px; color: #2c3e50; margin: 30px 0 8px 0; display: block; border-left: 4px solid #c0392b; padding-left: 10px; }}
...
.analysis-card.card-key {{
    background-color: #fff;
    border: 2px solid #fbc02d;
    border-left: 6px solid #fbc02d;
    border-bottom: 2px solid #fbc02d;
    box-shadow: 0 2px 8px rgba(251, 192, 45, 0.1);
}}

/* 영어 문장 박스 (줄간격 3.2로 확대 - 위아래 요소 표시 공간 확보) */
.eng-sent {{ 
    font-family: var(--font-eng); 
    font-size: 18px; 
    ...
    margin-bottom: 8px; 
    line-height: 3.2; /* 라벨과 단어 뜻을 위해 충분히 넓힘 */
    color: #000; 
    border-radius: 0 6px 6px 0; 
}}

.kor-sent {{ font-size: 14px; color: #444; padding-left: 5px; margin-bottom: 8px; line-height: 1.5; word-break: keep-all; }}
.summary-badge {{ display: inline-block; font-size: 11px; color: #e65100; background-color: #fff3e0; border: 1px solid #ffe0b2; padding: 1px 6px; border-radius: 10px; margin-left: 6px; font-weight: bold; vertical-align: 2px; }}
...
.scenario-box {{ background-color: #fff3e0; border-left: 5px solid #ff9800; padding: 15px; font-size: 13.5px; border-radius: 4px; line-height: 1.6; }}
    ...
    <div class="english-col">
        ...
        <div class="answer-box">
            ...
            <div class="ans-content">
                ...
                <strong>[소재 및 방향성]</strong><br>{clue_material}<br><br>
        ...
        <div class="scenario-box">
            <strong>🎯 지문 마킹 시뮬레이션:</strong><br>{scenario_sim}<br><br>
            <strong>🗣️ 수업 가이드:</strong><br>{scenario_guide}<br><br>
            <strong>💡 집중 팁:</strong> {scenario_tip}
        </div>
    </div>

    <div class="korean-col">
        <h3 style="margin-top:0; border-bottom:2px solid #333; padding-bottom:8px; font-size:18px;">
            📝 정밀 분석 (Deep Analysis) {header_badges}
        </h3>
...
def apply_vocab_style(text, vocab_list_dicts):
    ...
    for v in sorted_vocab:
        ...
        try:
            ...
            replacement = rf'<span class="vocab-container"><span class="vocab-text">\g<1></span><span class="vocab-sub">{meaning}</span></span>'
...
def convert_tags(text):
    ...
    # [Syntax Box Conversion] - Nesting supported by loop
...
def generate_html_from_json(json_str):
    try:
        ...
        processed_passage = convert_tags(passage_with_vocab)
        ...
        for i, opt_text in enumerate(vis_opts):
            ...
            if analysis_item:
                ...
                analysis_html = f"{badge_html}<span class='opt-reason'>{reason}</span>"
            ...
            <div class='option-item'>
                <div><span class='opt-text'>{opt_text}</span> {analysis_html}</div>
                {trans_html}
            </div>"""
        ...
        for s in sentences:
            ...
            eng = convert_tags(eng_with_vocab)
            kor = convert_tags(s.get('kor_translation', ''))
            ...
            <div class="{card_class}">
                <div class="eng-sent"><span style="font-weight:bold; margin-right:5px; color:#000;">{num_icon}</span>{eng_clean}</div>
                <div class="kor-sent">{kor} {summary_html}</div>
                <div class="syntax-tip"><span class="syntax-label">구문팁</span>{tip}</div>
            </div>"""

        targets_html = "<br>".join(
            [f"<li>{convert_tags(t)}</li>" for t in ans.get('clues', {}).get('target_sentences', [])])
        ...
        final_html = HTML_TEMPLATE.format(
            ...
            clue_material=convert_tags(ans.get('clues', {}).get('material', '')),
            clue_logic=convert_tags(ans.get('clues', {}).get('logic_flow', '')),
            logic_map_text=logic_map_text,
            scenario_sim=convert_tags(scene.get('simulation_text', '')),
        ...
        print("\n" + "=" * 40)
        print(f"✅ [성공] Final_v34 (Syntax Underline Style) 완료!")
