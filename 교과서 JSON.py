...
# =========================================================
# ⚙️ 설정 (입력 및 출력 폴더를 자유롭게 변경하세요!)
# =========================================================
# 변환할 파일이 있는 원본 폴더 (입력)
TARGET_FOLDER = "JSON"

# 변환된 JSON 파일이 저장될 폴더 (출력) - 지정한 폴더가 없으면 자동으로 생성됩니다.
OUTPUT_FOLDER = "3"
...
class HighSchoolParser:
    ...
    def parse_true_false(self):
        ...
        for line in lines:
            ...
            # 파트가 4개 (Question, Trans, Answer, Reason) 인지 확인
            if len(parts) >= 4:
    ...
    def parse_eng_def(self):
        ...
        for line in lines:
            parts = [p.strip() for p in line.split('|')]
            # 파트가 3개 (Word, Definition, Trans) 인지 확인
            if len(parts) >= 3:
    ...
    def parse_sentence_analysis(self):
        ...
        for block in blocks:
            ...
            if item: analysis_list.append(item)
    ...
    def parse_predicted_data(self):
        ...
        for item in items:
            ...
            for k, v in parts.items():
                if "reason" in k: reason_val = v
                if "trans" in k: trans_val = v
    ...
    def to_dict(self):
        ...
        return {
            ...
            "clues_data": {
                "subject_keywords": [k.strip() for k in self.search_line(r"Subject_Keywords:\s*(.*)", step6).split(',')
                                     if k.strip()],
...
def process_folder(target_folder, output_folder):
    if os.path.isabs(target_folder):
        input_path = target_folder
    else:
        input_path = os.path.join(os.path.expanduser("~"), "Desktop", target_folder)

    if not os.path.exists(input_path):
        print(f"❌ 원본 폴더를 찾을 수 없습니다: {input_path}")
        return

    if os.path.isabs(output_folder):
        out_path = output_folder
    else:
        out_path = os.path.join(os.path.expanduser("~"), "Desktop", output_folder)
    ...
    if not txt_files:
        print(f"⚠️ '{input_path}' 폴더 안에 변환할 txt 파일이 없습니다.")
        return

    print(f"총 {len(txt_files)}개의 파일을 변환 시작합니다...")
    print(f"▶ 원본 폴더: {input_path}")
    ...
    for filepath in txt_files:
        ...
        # 💡 여기에서 저장되는 파일의 확장자를 .json으로 완벽하게 변경했습니다.
        new_filename = filename.replace(".txt", ".json")
        output_filepath = os.path.join(out_path, new_filename)
        ...
        try:
            ...
            json_data = json.dumps(parser.to_dict(), ensure_ascii=False, indent=2)
            ...
            print(f"❌ 에러 발생 ({filename}): 파싱 중 문제가 발생했습니다. ({e})")
            error_count += 1

    print("-" * 40)
    print(f"🎉 작업 완료! (성공: {success_count}건, 실패: {error_count}건)")
